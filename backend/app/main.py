...
def build_payload(state: dict[str, Any], last_hit: dict[str, Any] | None = None) -> dict[str, Any]:
    enemy = state["enemy"]
...
def get_state() -> JSONResponse:
    with state_lock:
        ...
        update_records(STATE)
        save_state(STATE)
        return JSONResponse(build_payload(STATE))
...
def tap_enemy() -> JSONResponse:
    with state_lock:
        ...
        update_records(STATE)
        save_state(STATE)
        return JSONResponse(build_payload(STATE, {"damage": damage, "crit": crit}))
